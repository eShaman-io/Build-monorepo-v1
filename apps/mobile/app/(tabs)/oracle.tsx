...
export default function Oracle() {
  const [messages, setMessages] = useState<Message[]>([
    {
      ...
      role: "assistant",
      content:
        "Welcome, seeker. I am here to offer guidance and clarity. What weighs upon your heart today?",
      timestamp: new Date(),
    },
  ]);
  ...
  const generateOracleResponse = async (
    ...
    if (
      keywords.includes("decision") ||
      keywords.includes("choice") ||
      keywords.includes("choose")
    ) {
  ...
  const sendMessage = async () => {
    ...
    try {
      ...
      Alert.alert(
        "Connection Error",
        "Unable to reach the Oracle. Please try again.",
      );
    } finally {
      setIsLoading(false);
    }
