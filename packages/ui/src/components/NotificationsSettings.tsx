...
export function NotificationsSettings() {
  ...
  return (
    <View>
      ...
      {status !== 'granted' && (
        <Pressable onPress={requestPermissions} disabled={isSubmitting} className="rounded-lg bg-brand-secondary p-4">
          <Text className="text-center text-white">{isSubmitting ? 'Enabling...' : 'Enable Notifications'}</Text>
        </Pressable>
      )}
