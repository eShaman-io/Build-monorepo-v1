...
import type { Meditation } from '@esh/schemas';
...
export function MeditationList() {
  ...
  useEffect(() => {
    ...
    const unsubscribe = onSnapshot(q, (snapshot) => {
      const allMeditations = snapshot.docs.map(doc => ({ id: doc.id, ...doc.data() } as Meditation));
  ...
  return (
    <div className="space-y-4">
      <h2 className="mb-4 text-center text-4xl font-bold">Guided Meditations</h2>
      {meditations.map((meditation) => (
        <GlassCard key={meditation.id} className="p-6">
          <h3 className="text-2xl font-bold">{meditation.title}</h3>
          <p className="mb-4 text-brand-neutral-dark">{meditation.description}</p>
          <OrbButton
