...
const scaffoldRouteTool = {
  name: "scaffoldRoute",
  description:
    "Create a Next.js route handler (api) or page with optional Zod schema and test.",
  inputSchema: {
    type: "object",
    properties: {
      appDir: {
        type: "string",
        description: "Path to Next.js app dir",
        default: DEFAULT_APP_DIR,
      },
      route: {
        type: "string",
        description: "Route path (e.g., /api/ping or /about)",
      },
  ...
  handler: async ({
    ...
    const handlerPath = isApi
      ? path.join(baseDir, "route.ts")
...
`;
    ...
    const write = await run(
      `powershell -Command "Set-Content -Path '${handlerPath}' -Value @'\n${handlerSource.replace(/'/g, "''")}\n'@"`,
    );
    ...
    if (withTest) {
      ...
      const testFile = path.join(
        testDir,
        `${normalized.replace(/[\\/]/g, "_")}.test.ts`,
      );
...
`;
      testWrite = await run(
        `powershell -Command "Set-Content -Path '${testFile}' -Value @'\n${testSrc.replace(/'/g, "''")}\n'@"`,
      );
    }
...
const firebaseDeployTool = {
  ...
  inputSchema: {
    ...
    properties: {
      ...
      project: {
        type: "string",
        description: "Firebase project alias or ID (optional)",
      },
    },
  },
  handler: async ({ only = "hosting,functions", project }) => {
    ...
    return {
      ...res,
      hint: "If auth fails, run `pnpm exec firebase login` or add firebase-tools locally.",
    };
  },
};
...
const fixLintTool = {
  ...
  handler: async () => {
    const lint = await run(
      `pnpm run -w lint --if-present || echo "no lint script"`,
    );
...
const typecheckTool = {
  ...
  inputSchema: { type: "object", properties: {} },
  handler: async () =>
    run(`pnpm run -w typecheck --if-present || pnpm exec tsc -b --pretty`),
};
...
const runTestsTool = {
  ...
  handler: async ({ watch = false }) =>
    run(watch ? `pnpm test -w -- --watch` : `pnpm test -w`),
};
...
const bumpDepsTool = {
  ...
  handler: async () => {
    const check = await run(
      `pnpm -w outdated || echo "No outdated packages or command failed."`,
    );
...
console.error(
  "[local-tools] MCP server running via stdio. Root:",
  WORKSPACE_ROOT,
);
